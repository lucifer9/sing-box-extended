...
Optional additional REALITY authentication. Supply the server's **1952-byte ML-DSA-65 public verification key**, encoded as URL-safe Base64 without padding. Omit this field or leave it empty to disable additional verification. Invalid encoding or key length is rejected when constructing the client configuration.

When configured, both the existing REALITY authentication and the ML-DSA-65 signature must succeed. A wrong key, missing signature, or invalid signature rejects the connection; an ordinary trusted TLS certificate does not grant proxy access. The server must support and enable ML-DSA-65 signing (for example, Xray v26.9.9).

This key is distinct from the X25519 `public_key` and the X25519MLKEM768 handshake key share. It is not a seed, private key, or PEM certificate. `sing-box generate reality-keypair` generates X25519 keys, not this verification key. The field is outbound-only; it does not enable server-side signing. Use `mldsa65_verify`, not Xray's `mldsa65Verify` spelling.
